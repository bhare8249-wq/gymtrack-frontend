...
function useStorage(username) {
  ...
  useEffect(() => {
    try { const r = localStorage.getItem(key); setData(r ? JSON.parse(r) : { workouts: [], bodyweight: [] }); }
    catch { setData({ workouts: [], bodyweight: [] }); }
  }, [key]);
...
export default function App() {
  ...
  const navItem = (v, icon, label) => (
    <button onClick={() => { if (v === "log" && !workout) setWorkout({ date: todayISO(), startTime: Date.now(), exercises: [] }); setView(v); }}
      style={{ flex: 1, background: "transparent", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, color: view === v ? accent : t.textMuted, padding: "10px 0", transition: "color 0.2s" }}>
            ...
            {isEditing ? (
              <div>
                ...
                {(() => {
                  ...
                  const [secTab, setSecTab] = useState("email"); // "email" | "password"
                  ...
                  const handleEmailChange = () => {
                    ...
                    if (Object.entries(users).some(([u, d]) => u !== authedUser && d.email === newEmail)) {
                      setSecMsg({ type: "error", text: "That email is already associated with another account." }); return;
                    }
                    // Update email, mark unverified, show verification screen
                  ...
                  const handlePasswordChange = () => {
                    ...
                    // Update password, mark unverified, show verification screen
                    users[authedUser] = { ...user, passwordHash: btoa(newPw), verified: false };
                    saveUsers(users);
                    setSecVerify(true);
                    setSecMsg({ type: "success", text: `Security code sent to ${user.email}` });
                  };
                  ...
                  if (secVerify) return (
                    <div style={{ marginTop: 20, background: `${accent}10`, border: `1px solid ${accent}44`, borderRadius: 12, padding: "20px 16px", textAlign: "center" }}>
                      ...
                      }} style={{ background: accent, color: "#000", border: "none", borderRadius: 9, padding: "10px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer", marginBottom: 8, width: "100%" }}>
                  ...
                  return (
                    <div style={{ marginTop: 20 }}>
                      <button onClick={() => { setShowSecurity(s => !s); setSecMsg(null); }} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", background: t.surfaceHigh, border: `1px solid ${t.border}`, borderRadius: 10, padding: "12px 14px", cursor: "pointer", color: t.text, fontWeight: 600, fontSize: 13 }}>
                        <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <Icon name="shield" size={14} /> Security Settings
                        </span>
                        <span style={{ color: t.textMuted, fontSize: 12, transform: showSecurity ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>▼</span>
                      </button>
